...
# --- Paths ---
nodes_csv = r"C:\Users\\Thesis code\manual_nodes_updated.csv"
edges_csv = r"C:\Users\\Thesis code\edges_updated.csv"
...
def resolve_file_path(basename: str, folder: str) -> list:
    ...
    if basename.startswith("*"):  # match files ending with pattern
        suffix = basename[1:]
        return [os.path.join(folder, f) for f in os.listdir(folder) if f.endswith(suffix + ".csv")]
    elif basename.endswith("*"):  # match files starting with pattern
        prefix = basename[:-2]
        return [os.path.join(folder, f) for f in os.listdir(folder) if f.startswith(prefix) and f.endswith(".csv")]
    else:
        file_path = os.path.join(folder, f"{basename}.csv")
        return [file_path] if os.path.exists(file_path) else []


# --- Helper: Sum how much reqflex exceeds actflex ---
def sum_reqflex_minus_actflex(path: str) -> float:
    """
    Read CSV and return the total amount by which reqflex (second-to-last column)
    exceeds actflex (last column), summed across all rows.
    """
    df = pd.read_csv(path)
    if df.shape[1] < 2:
        return 0.0

    actflex_col = df.columns[-1]      # last column
    reqflex_col = df.columns[-2]      # second-to-last column

    actflex = pd.to_numeric(df[actflex_col], errors="coerce")
    reqflex = pd.to_numeric(df[reqflex_col], errors="coerce")

    # Calculate positive differences only
    differences = (reqflex - actflex).clip(lower=0)
...
for node in nodes:
    ...
    for fp in file_paths:
        file_diff = sum_reqflex_minus_actflex(fp)
...
# Set manual color scale limits
manual_max = 40  # <-- your chosen upper bound
vmin = 0.0         # or another lower bound you prefer
...
cbar.set_label("Summed remaining congestion percentage", fontsize=10)
...
# %%
## GREEN part from here on


# --- Helper: Sum how much reqflex exceeds actflex ---
def sum_actflex_minus_reqflex(path: str) -> float:
    """
    Read CSV and return the total amount by which reqflex (second-to-last column)
    exceeds actflex (last column), summed across all rows.
    """
    ...
    # Calculate positive differences only
    differences = (actflex - reqflex)#.clip(lower=0)
...
for node in nodes:
    ...
    total_diff = 0.0
    for fp in file_paths:
        file_diff = sum_actflex_minus_reqflex(fp)
        print(f"    [DETAIL] Processing file: {os.path.basename(fp)}, Shortage (actflex - reqflex) = {file_diff}")
        total_diff += file_diff
...
# Set manual color scale limits
manual_max = 40  # <-- your chosen upper bound
vmin = -40         # or another lower bound you prefer
vmax = manual_max  # instead of max(values)
...
sm.set_array([])
cbar = plt.colorbar(sm, ax=ax, fraction=0.03, pad=0.04)
cbar.set_label("Summed percentual overshoots", fontsize=10)
cbar.ax.tick_params(labelsize=8)
